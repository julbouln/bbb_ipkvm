/**
 * simple VNC SDL client with audio support
 * based on SDLvncviewer.c example
 */
...
static rfbBool rfb_audio_handle(rfbClient* client, rfbServerToClientMsg* message)
{
	audio_msg_t msg;
	char *buf;

	if(message->type != audioType)
		return FALSE;

	if(!ReadFromRFBServer(client, ((char*)&msg)+1, sizeof(msg)-1))
		return TRUE;

	msg.size = rfbClientSwap16IfLE(msg.size);
	buf = malloc(msg.size);

	if(!ReadFromRFBServer(client, buf, msg.size)) {
		free(buf);
		return TRUE;
	}

	if(SDL_QueueAudio(audiodev, buf, msg.size) < 0) {
		fprintf(stderr, "cannot queue audio data : %s\n",SDL_GetError());
	}

	unsigned int queued = SDL_GetQueuedAudioSize(audiodev);
	if(queued > 38400) { // 200ms max, 48000*2*2 = 192000 = 1sec
		printf("audio out of sync: %d\n", queued);
...
static rfbClientProtocolExtension audio_extension = {
	rfb_audio_encoding,		/* encodings */
	NULL,				/* handleEncoding */
	rfb_audio_handle,	/* handleMessage */
	NULL,				/* next extension */
	NULL,				/* securityTypes */
